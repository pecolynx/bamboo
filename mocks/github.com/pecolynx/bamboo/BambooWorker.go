// Code generated by mockery v2.34.2. DO NOT EDIT.

package mocks

import (
	context "context"

	mock "github.com/stretchr/testify/mock"
)

// BambooWorker is an autogenerated mock type for the BambooWorker type
type BambooWorker struct {
	mock.Mock
}

type BambooWorker_Expecter struct {
	mock *mock.Mock
}

func (_m *BambooWorker) EXPECT() *BambooWorker_Expecter {
	return &BambooWorker_Expecter{mock: &_m.Mock}
}

// Run provides a mock function with given fields: ctx
func (_m *BambooWorker) Run(ctx context.Context) error {
	ret := _m.Called(ctx)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context) error); ok {
		r0 = rf(ctx)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// BambooWorker_Run_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Run'
type BambooWorker_Run_Call struct {
	*mock.Call
}

// Run is a helper method to define mock.On call
//   - ctx context.Context
func (_e *BambooWorker_Expecter) Run(ctx interface{}) *BambooWorker_Run_Call {
	return &BambooWorker_Run_Call{Call: _e.mock.On("Run", ctx)}
}

func (_c *BambooWorker_Run_Call) Run(run func(ctx context.Context)) *BambooWorker_Run_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context))
	})
	return _c
}

func (_c *BambooWorker_Run_Call) Return(_a0 error) *BambooWorker_Run_Call {
	_c.Call.Return(_a0)
	return _c
}

func (_c *BambooWorker_Run_Call) RunAndReturn(run func(context.Context) error) *BambooWorker_Run_Call {
	_c.Call.Return(run)
	return _c
}

// NewBambooWorker creates a new instance of BambooWorker. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewBambooWorker(t interface {
	mock.TestingT
	Cleanup(func())
}) *BambooWorker {
	mock := &BambooWorker{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
